...
import co.antonis.gwt.example.client.generated.SerGwtJsonTypes;
import co.antonis.gwt.example.client.generated.SerGwtJson_Sample;
...
public class UiPanelRequest extends Composite {
    ...
    public static <T> T convertPojo(String json, Class<T> clazz) {
        return SerGwtJsonTypes.fromJson(clazz, json);
    }
    ...
    public void buttonSubmitClick(ClickEvent event) {
        ...
        if (uiCheckSendAsParam1.getValue()) {
            ...
            PojoParent pojo = SerGwtJson_Sample.toPojoParent(uiTextResponseBody.getText());

            Log.info("Converted POJO to JSON");
            JSONObject jsonConverted = SerGwtJson_Sample.fromPojoParent(pojo);
